...
{
    private readonly GreaterThanValidator _validator = new();

    [Fact]
    public void ValidatorType_Should_Return_gt()
    {
        // Act
    ...
    public void Validate_With_IntegerValues_Should_ReturnCorrectResult(int value, object conditionValue, bool expectedValid)
    {
        ...
        if (!expectedValid)
        {
            result.Message.Should().Contain("Value must be greater than threshold");
            result.Message.Should().Contain("Actual value:");
            result.Message.Should().Contain("Expected value:");
            result.ValidatorType.Should().Be("gt");
        }
        else
        {
    ...
    [Theory]
    [InlineData(3.2, 5.0, false)] // 3.2 < 5.0
    [InlineData(5.2, 5.0, true)] // 5.2 > 5.0
    [InlineData(3.2, 5, false)] // 3.2 < 5
    [InlineData(6.2, 5, true)] // 6.2 > 5
    public void Validate_With_DoubleToIntComparison_Should_ThrowExceptionForIntToDouble(double value, object conditionValue, bool expectedValid)
    {
        // Arrange
        var condition = new ValidatorCondition
        {
    ...
    public void Validate_With_NonComparableValue_Should_Fail_WithProperError()
    {
        ...
        result.Message.Should().Contain("Value for 'gt' validator must be comparable");
        result.ValidatorType.Should().Be("gt");
    }
    ...
    public void Validate_With_Failure_Should_Include_CustomErrorMessage()
    {
        ...
        var condition = new ValidatorCondition
        {
            Validator = "gt",
            Value = 10,
            Message = "Custom greater than error message"
        };
        ...
        // Assert
        result.IsValid.Should().BeFalse();
        result.Message.Should().Contain("Custom greater than error message");
        result.Message.Should().Contain("Actual value:");
        result.Message.Should().Contain("Expected value:");
        result.ValidatorType.Should().Be("gt");
    ...
    public void Validate_With_DateTimeValues_Should_ReturnCorrectResult()
    {
        ...
        result2.Message.Should().Contain("Value must be greater than threshold");
        result2.Message.Should().Contain("Actual value:");
        result2.Message.Should().Contain("Expected value:");
        result2.ValidatorType.Should().Be("gt");
    }
