...
public sealed class FileOutputFormatterAdditionalTests : IDisposable
{
    private readonly string _testDirectory;

    public FileOutputFormatterAdditionalTests()
    {
        _testDirectory = DirectoryUtility.CreateTempDirectory("fileoutputformatter-test");
    }
    ...
    [Fact]
    public async Task FormatAsync_With_SuccessResult_Should_Write_TextFile()
    {
        // Arrange
        var filePath = Path.Combine(_testDirectory, "output.txt");
        var formatter = new FileOutputFormatter(filePath);
    ...
    [Fact]
    public async Task FormatAsync_With_ErrorResult_Should_Write_Error()
    {
        // Arrange
        var filePath = Path.Combine(_testDirectory, "output.txt");
        var formatter = new FileOutputFormatter(filePath);
    ...
    [Fact]
    public async Task FormatAsync_With_ErrorResult_And_Exception_Should_Write_Exception()
    {
        // Arrange
        var filePath = Path.Combine(_testDirectory, "output.txt");
        var formatter = new FileOutputFormatter(filePath);
        var exception = new Exception("Inner error");
        var result = RuleEngineResult.CreateError("Test error", exception);
    ...
    public async Task FormatAsync_With_NonExistentDirectory_Should_Create_Directory()
    {
        ...
        var filePath = Path.Combine(subDir, "output.txt");
        var formatter = new FileOutputFormatter(filePath);
        var result = RuleEngineResult.CreateSuccess(new FileValidationResult("test.json", []));
    ...
    [Fact]
    public async Task FormatAsync_With_EmptyResults_Should_Write_Summary()
    {
        // Arrange
        var filePath = Path.Combine(_testDirectory, "output.txt");
        var formatter = new FileOutputFormatter(filePath);
        var result = RuleEngineResult.CreateSuccess([]);
