...
public sealed class InValidator : BaseValidator<object>
{
    ...
    public override IReadOnlySet<Common.ValueType> SupportedValueTypes { get; } = new HashSet<Common.ValueType>
        { Common.ValueType.String, Common.ValueType.Array, Common.ValueType.JsonElement };
    ...
    public override ValidationResult Validate(object? value, ValidatorCondition condition)
    {
        ...
        if (!conditionTyped.TryGetStringArray(out var allowedValues))
        {
            ...
            if (condition.Value is not System.Text.Json.JsonElement { ValueKind: System.Text.Json.JsonValueKind.Array } jsonElement)
            {
                return CreateFailure(
                    ...
                    $"Actual value type: {condition.Value?.GetType().Name ?? "null"}. " + "Please provide an array of allowed values.", string.Empty,
        ...
        return CreateFailure(condition.Message, string.Empty, value, $"one of: {allowedValuesStr}");
    }
}