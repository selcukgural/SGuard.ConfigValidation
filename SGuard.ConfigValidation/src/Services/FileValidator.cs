...
public sealed class FileValidator : IFileValidator
{
    ...
    /// <returns>A task that represents the asynchronous operation. The task result contains a <see cref="FileValidationResult"/> containing all validation results for the file.</returns>
    ...
    /// };
    /// 
    /// var result = await fileValidator.ValidateFileAsync("appsettings.Production.json", rules, appSettings);
    /// 
    /// if (result.IsValid)
    /// {
    ...
    /// </code>
    /// </example>
    public async Task<FileValidationResult> ValidateFileAsync(string filePath, List<Rule> applicableRules, Dictionary<string, object>? appSettings, CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        return await ValidateFileInternalAsync(filePath, applicableRules, appSettings, cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        return ValidateFileInternal(filePath, applicableRules, appSettings);
    }

    /// <summary>
    /// Internal validation logic for asynchronous operations.
    /// </summary>
    private async Task<FileValidationResult> ValidateFileInternalAsync(string filePath, List<Rule> applicableRules, Dictionary<string, object>? appSettings, CancellationToken cancellationToken)
    {
...
#pragma warning disable CS8602, CS8767 // Null check is intentional for runtime safety (deserialization may produce nulls)
            if (rule == null)
#pragma warning restore CS8602, CS8767
            {
            ...
            {
                cancellationToken.ThrowIfCancellationRequested();
                var conditionResults = await ValidateConditionAsync(condition, appSettings, cancellationToken).ConfigureAwait(false);
    ...
    /// <summary>
    /// Internal validation logic for synchronous operations.
    /// </summary>
    private FileValidationResult ValidateFileInternal(string filePath, List<Rule> applicableRules, Dictionary<string, object>? appSettings)
    {
        if (string.IsNullOrWhiteSpace(filePath))
        {
            _logger.LogError("File validation failed: File path is null or empty");
...
#pragma warning restore CS8602, CS8767
    ...
    /// <summary>
    /// Validates a condition against app settings asynchronously.
    /// </summary>
    private async Task<List<ValidationResult>> ValidateConditionAsync(Condition condition, Dictionary<string, object> appSettings, CancellationToken cancellationToken)
    {
    ...
    /// <summary>
    /// Validates a condition against app settings.
    /// </summary>
